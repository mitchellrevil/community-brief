...
import { AlertCircle, Loader2, Maximize2, MessageCircle, Minimize2, Send, X } from 'lucide-react';
...
/**
 * Props for DesktopChatPanel component
 */
...
/**
 * Desktop-specific chat panel component.
 * Renders a floating card with minimize/maximize functionality.
 * Memoized to prevent unnecessary re-renders.
 */
export const DesktopChatPanel = memo(function DesktopChatPanelView({
  messages,
  input,
  setInput,
  isLoading,
  ...
  return (
    <Card
      className={cn(
        ...
        isExpanded
          ? 'bottom-6 right-6 w-[min(90vw,48rem)] h-[min(85vh,42rem)]'
          : 'bottom-6 right-6 w-[min(92vw,28rem)] h-[min(70vh,32rem)]'
      )}
    >
      <CardHeader className="pb-3 border-b">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            <Button onClick={onToggleExpanded} variant="ghost" size="sm" aria-label={isExpanded ? "Minimize chat panel" : "Maximize chat panel"}>
              {isExpanded ? <Minimize2 className="h-4 w-4" aria-hidden="true" /> : <Maximize2 className="h-4 w-4" aria-hidden="true" />}
            </Button>
            <Button onClick={onClose} variant="ghost" size="sm" aria-label="Close chat panel">
              <X className="h-4 w-4" aria-hidden="true" />
            </Button>
          </div>
        </div>
      </CardHeader>
      <CardContent className="p-0 h-[calc(100%-4rem)] overflow-hidden flex flex-col">
