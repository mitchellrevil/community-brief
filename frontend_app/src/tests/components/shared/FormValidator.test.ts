 ...
 * - normalizeOptions: array/string options normalization
 * - normalizeChecked: checkbox state normalization
...
describe('FormValidator', () => {
  describe('validateField', () => {
    ...
    describe('markdown fields', () => {
      it('should return error for required empty markdown field', () => {
        const error = validateField({ name: 'content', type: 'markdown', label: 'Content', required: true }, '');
        expect(error).toBeTruthy();
        
        const noError = validateField({ name: 'content', type: 'markdown', label: 'Content', required: true }, '# Title');
        expect(noError).toBeNull();
      });
    });

    describe('textarea fields', () => {
      it('should return error for required empty textarea', () => {
        const error = validateField({ name: 'notes', type: 'textarea', label: 'Notes', required: true }, '');
        expect(error).toBeTruthy();
        
        const noError = validateField({ name: 'notes', type: 'textarea', label: 'Notes', required: true }, 'Some notes here');
        expect(noError).toBeNull();
      });
    });
  });
  ...
  describe('normalizeOptions helper', () => {
    it('should handle array options', () => {
      expect(normalizeOptions(['a', 'b', 'c'])).toEqual(['a', 'b', 'c']);
    });

    it('should handle comma-separated string options', () => {
      expect(normalizeOptions('a, b, c')).toEqual(['a', 'b', 'c']);
    });
  ...
  describe('normalizeChecked helper', () => {
    it('should return true for true', () => {
      expect(normalizeChecked(true)).toBe(true);
    });

    it('should return false for false', () => {
      expect(normalizeChecked(false)).toBe(false);
    });

    it('should return false for indeterminate', () => {
      expect(normalizeChecked('indeterminate')).toBe(false);
    });
  });
