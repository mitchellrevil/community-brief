...
vi.mock('@/components/ui/editable-display-name', () => ({
  EditableDisplayName: ({ job }: any) => (
    <span data-testid="editable-display-name">{job.display_name || 'Recording'}</span>
  ),
}));
...
describe('RecordingHeader', () => {
  ...
  describe('basic rendering', () => {
    it('should render recording title', () => {
      render(
        <RecordingHeader recording={mockRecording as any} isTinyScreen={false} />
      );

      expect(screen.getByTestId('editable-display-name')).toHaveTextContent('Test Recording');
    });
    ...
    it('should render navigation back button', () => {
      ...
      expect(screen.getByRole('link')).toHaveAttribute('href', '/audio-recordings');
    });
  });
  ...
  describe('popover integration', () => {
    it('should render TranscriptionStatusPopover when showTranscriptionPopover is true', () => {
      ...
      expect(screen.getByText(/your audio is being processed/i)).toBeInTheDocument();
    });
    ...
    it('should not render popover content when showTranscriptionPopover is false', () => {
      ...
      expect(screen.queryByText(/your audio is being processed/i)).not.toBeInTheDocument();
    });
  ...
  describe('responsive behavior', () => {
    it('should adapt layout for tiny screens', () => {
      render(
        <RecordingHeader recording={mockRecording as any} isTinyScreen={true} />
      );
