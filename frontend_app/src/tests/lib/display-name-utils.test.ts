...
describe("Display Name Utilities", () => {
  describe("getDisplayName", () => {
    ...
    test("should support display_name from API responses", async () => {
      const { getDisplayName } = await import("@/lib/display-name-utils");

      const job = { display_name: "API Recording", file_name: "audio.mp3" };
      expect(getDisplayName(job)).toBe("API Recording");
    });
